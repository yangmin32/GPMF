...
from examples.battle_model.senario_battle import play3
from examples.battle_model.senario_battle import play4
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, choices={'ac', 'mfac', 'mfq', 'rnnMFQ', 'rnnIL', 'il', 'pomfq','gamfq','gamfq1'}, help='choose an algorithm from the preset', required=True)
    ...
    if args.algo in ['mfq', 'mfac', 'rnnMFQ', 'pomfq','gamfq','gamfq1']:
        use_mf = True
    else:
        use_mf = False
    ...
    if args.algo == 'pomfq':
        ...
        if args.algo == 'gamfq':
            ...
            if args.algo == 'gamfq1':
                runner = tools.Runner(sess, env, handles, args.map_size, args.max_steps, models, play4,
                            render_every=args.save_every if args.render else 0, save_every=args.save_every, tau=0.01, log_name=args.algo,
                            log_dir=log_dir, model_dir=model_dir, train=True)
            else:
    ...
    for k in range(start_from, start_from + args.n_round):
        ...
        total_reward ,nums = runner.run(eps, k)

        with open('mfac1_multibattle_r6.csv', 'a') as myfile:
            myfile.write('{0},{1},{2},{3}\n'.format(k, total_reward[0],total_reward[1],nums))
