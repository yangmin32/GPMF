...
class ValueNet:
    def __init__(self, sess, env, handle, name, update_every=5, use_mf=False, learning_rate=1e-4, tau=0.005, gamma=0.95, maxelements=20):
        ...
        self.handle = handle
        self.view_space = env.get_view_space(handle)   #n * view_width * view_height * n_channel   n*13*13*7
        assert len(self.view_space) == 3    #断言函数        
        self.feature_space = env.get_feature_space(handle)    #n * feature_size   
