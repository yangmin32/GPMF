...
class GraphAttention(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, negative_slope, num_heads=1, bias=True, self_loop_type=1, average=False, normalize=False):
        super(GraphAttention, self).__init__()
        """
        Initialization method for the graph-attentional layer

        Arguments:
            in_features (int): number of features in each input node 每个输入节点中的特征数
            out_features (int): number of features in each output node 每个输出节点中的特征数
            dropout (int/float): dropout probability for the coefficients 系数的丢失概率
            negative_slope (int/float): control the angle of the negative slope in leakyrelu 控制leakyrelu中负斜率的角度
            number_heads (int): number of heads of attention 关注头数  默认为1
            bias (bool): if adding bias to the output 如果向输出添加偏置
            self_loop_type (int): 0 -- force no self-loop; 1 -- force self-loop; other values (2)-- keep the input adjacency matrix unchanged
            0 -- 强制不自循环， 1 -- 强制自循环 ， (2)--保持输入邻接矩阵不变   
            average (bool): if averaging all attention heads 如果平均所有注意力
            normalize (bool): if normalizing the coefficients after zeroing out weights using the communication graph 如果在使用通信图将权重归零后对系数进行归一化
        """
