...
from examples.battle_model.senario_battle import battle2
from examples.battle_model.senario_battle import battle2_change
from examples.battle_model.senario_battle import battle3
from examples.battle_model.senario_battle import battle3_change
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, choices={'ac', 'mfac', 'mfq', 'il', 'rnnIL', 'rnnMFQ', 'pomfq','gamfq','gamfq1','gamfq2'}, help='choose an algorithm from the preset', required=True)
    parser.add_argument('--oppo', type=str, choices={'ac', 'mfac', 'mfq', 'il', 'rnnIL', 'rnnMFQ', 'pomfq','gamfq','gamfq1','gamfq2'}, help='indicate the opponent model')
    ...
    runner = tools.Runner(sess, env, handles, args.map_size, args.max_steps, models, battle2_change, pomfq_position, render_every=0)
    win_cnt = {'main': 0, 'opponent': 0}

    for k in range(0, args.n_round):
        total_rewards , nums= runner.run(0.0, k, win_cnt=win_cnt)
        # with open('mfq_mfac_storepoints_multibattle.csv', 'a') as myfile:
        #     myfile.write('{0},{1},{2},{3},{4}\n'.format(k, total_rewards[0], total_rewards[1],nums[0],nums[1]))
